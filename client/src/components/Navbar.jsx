  ...
  return (
    <div className="flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6">
      ...
      <div className="sm:flex hidden flex-row justify-end gap-4">
        <CustomButton 
          btnType="button"
          title={address == basesAddress ? 'Create a campaign' : address ? address.slice(0,6) : 'connect'}
          styles={address == basesAddress ? 'bg-[#1dc071]' : 'bg-[#8c6dfd]'}
          handleClick={() => {
            if(address == basesAddress) navigate('create-campaign')
            else connect()
          }}
        />
